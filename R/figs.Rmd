...
dirNew='/home/laurie/MEGAsync/papers/tuna-mse/data'
```
...
---------------------------------------
```{r popPar}
load(file.path(dirNew,"design.RData"))
load(file.path(dirNew,"priors.RData"))
...
```{r transfer}
load(file.path(dirNew,"trans.RData"))
load(file.path(dirNew,"omKey.RData"))

ggplot(trans)+
  geom_path(aes(ftar,ageC))+
  facet_wrap(ar~OM,scale="free")+
  xlab("")+ylab("")+theme_bw()
```
...
```{r transfer2}
load(file.path(dirNew,"spc.RData"))
load(file.path(dirNew,"omKey.RData"))
...
load(file.path(dirNew,"om.RData"))
...
save(res,file=file.path(dirNew,"res.RData"))
```
  
```{r Smry,echo=FALSE}
load(file.path(dirNew,"res.RData"))
...
```{r}
load(file.path(dirNew,"options.RData"))
load(file.path(dirNew,"design.RData"))
load(file.path(dirNew,"omKey.RData"))
load(file.path(dirNew,"priors.RData"))
load(file.path(dirNew,"trans.RData"))
...
```{r yld}
ggplot(subset(trans,variable=="yld"))+
  geom_path(aes(xvar,value,col=factor(OM)))+
  geom_point(aes(Effort,Yield,col=factor(OM)),data=res)
```

```{r aav2}
ggplot(subset(trans,variable=="aav"))+
  geom_path(aes(xvar,value,col=factor(OM)))+
  geom_point(aes(Effort,aaY,col=factor(OM)),data=res)
```

```{r tranf-yld}
ggplot(subset(trans,variable!="ftar"))+
  geom_path(aes(xvar,value,col=factor(OM)))+
...
ggplot(subset(trans,variable=="ssb"))+
  geom_path(aes(xvar,value,col=factor(OM+ar)))+
  geom_point(aes(Effort,1-Green,col=factor(OM)),data=res)+
  xlab("Effort")+ylab("")
```
